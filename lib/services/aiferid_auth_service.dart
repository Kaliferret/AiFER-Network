...
class AiFERiDAuthService {
  ...
  // ─────────────────────────────────────────────────────────────
  // Legacy-API shim methods
  //
  // The pre-Phase-4 service exposed a different surface. These shims
  // delegate to the new Claude-built API so existing callers compile.
  // Phase 5 will refactor the call sites and remove these shims.
  // ─────────────────────────────────────────────────────────────

  /// Legacy: wallet-address-based login. Maps to [authenticateWithWallet].
  Future<Map<String, dynamic>> authenticateWithAiFERiD({
    required String aiFERiD,
    String? signatureProof,
  }) async {
    try {
      final result = await authenticateWithWallet(
        aiFERiD,
        signatureProof ?? _fallbackSignature(aiFERiD),
        <String, dynamic>{'source': 'legacy-shim'},
      );
      if (result.success) {
        return {
          'success': true,
          ...
          'aiferid': aiFERiD,
          'auth_type': result.authType.toString(),
  ...
  /// Legacy: create a new AiFERiD account.
  Future<Map<String, dynamic>> createAiFERiDAccount({
    String? aiFERiD,
    String? displayName,
    String? accountName,
    String? userEmail,
    ...
    try {
      final effectiveName = displayName ??
          ...
          fullName ??
          aiFERiD ??
          'Ferret-${DateTime.now().millisecondsSinceEpoch}';
      final result = await createAnonymousSession(
        ferretName: effectiveName,
      );
      return {
        'success': result.success,
        'aiferid': result.ferretId ?? aiFERiD ?? effectiveName,
        'user_id': result.userId,
        'session_id': result.sessionId,
  ...
  /// Legacy: return the active AiFERiD string, or null.
  String? getCurrentAiFERiD() => getCurrentUser()?.walletAddress;

  /// Legacy: is there a live session? Async-friendly for older callers.
  Future<bool> isAuthenticated() async => getCurrentUser() != null;

  /// Legacy: sign out current user. Logs out ALL sessions to be safe.
  Future<void> signOut() async {
    await logoutAll();
  }

  /// Legacy: basic format check — AiFERiD strings are non-empty alphanumeric.
  bool isValidAiFERiDFormat(String aiFERiD) {
    if (aiFERiD.isEmpty || aiFERiD.length < 4) return false;
    return RegExp(r'^[A-Za-z0-9\-_]{4,}$').hasMatch(aiFERiD);
  }

  String _fallbackSignature(String aiFERiD) {
    final bytes = utf8.encode('legacy-shim-$aiFERiD');

...
class FERFrequencyHopping {
  ...
  final Map<String, double> _channelQuality = {};
  ...
  double _estimateCongestion(double frequency) {
    ...
    final baseCongestion = Random().nextDouble() * 0.3; // 0-30% base congestion
  ...
  Future<FrequencyMetrics> _scanFrequency(double frequency) async {
    ...
    return FrequencyMetrics(
      signalStrength: 60 + Random().nextDouble() * 40, // 60-100%
      interferenceLevel: Random().nextDouble() * 20, // 0-20%
      congestionLevel: Random().nextDouble() * 30, // 0-30%
